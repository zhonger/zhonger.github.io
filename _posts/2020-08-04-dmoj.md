...
&emsp;&emsp;由于 DMOJ 设计时数据库用了较长的索引，此处还需要有进一步对数据库进行设置（官方文档中没有提到，可能是操作系统不同的原因）。在 /etc/mysql/mariadb.conf.d/50-server.conf 文件的 innodb 注释位置添加如下三行内容，修改完成后重启Mysql数据库生效配置。除此在外，还要在之后下载的 DMOJ 主项目代码文件 site/manage.py 中添加如下两行。
